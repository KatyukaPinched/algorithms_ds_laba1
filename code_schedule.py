...
polinom = np.poly1d(p_Insertion)
print("Уравнение регрессионной кривой для вставками — ", polinom)
p_Selection = np.polyfit(x_arr_Selection, time_arr_Selection, 2)
polinom = np.poly1d(p_Selection)
print("Уравнение регрессионной кривой для выбором — ", polinom)
p_Bubble = np.polyfit(x_arr_Bubble, time_arr_Bubble, 2)
polinom = np.poly1d(p_Bubble)
print("Уравнение регрессионной кривой для сортировки пузырьком — ", polinom)'''
p_Merge = np.polyfit(x_arr_Merge, time_arr_Merge, 2)
polinom = np.poly1d(p_Merge)
print("Уравнение регрессионной кривой для сортировки слиянием — ", polinom)
p_Shell = np.polyfit(x_arr_Shell, time_arr_Shell, 2)
polinom = np.poly1d(p_Shell)
print("Уравнение регрессионной кривой для сортировки Шелла — ", polinom)
p_Quick = np.polyfit(x_arr_Quick, time_arr_Quick, 2)
polinom = np.poly1d(p_Quick)
print("Уравнение регрессионной кривой для быстрой сортировки — ", polinom)
p_Heap = np.polyfit(x_arr_Heap, time_arr_Heap, 2)
polinom = np.poly1d(p_Heap)
print("Уравнение регрессионной кривой для сортировки кучей — ", polinom)
